...
declare interface AnimeStatic {
  ...
  (params?: Partial<AnimInput>): AnimInstance;
   ...
   */
  path(path: any, percent: number): (prop: string) => { el: SVGPathElement[], property: string, totalLength: number };
   ...
   */
  bezier(x1: number, x2: number, y1: number, y2: number): (x: number) => number;


  /**
   * Create a timeline to synchronise other Anime instances.
   */
  timeline(params?: Partial<TimelineInput>): TimelineInstance;
...
export interface AnimInput extends Targets, AnimatableProperties {
}
 ...
 * Instance
 */
export interface AnimInstance extends
...
declare type animCallback = (instance: AnimInstance) => {};
...
export interface TimelineInput extends AnimInput, instanceParams {

}

export interface TimelineInstance extends AnimInstance {
  add(params: Partial<AnimInput>, timeOffset?: string | number): any;
}
